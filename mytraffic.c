...
static ssize_t device_read(struct file *f, char __user *buf, size_t len, loff_t *off)
{
    ...
    int red_val, yellow_val, green_val;
    ...
    red_val = gpio_get_value(GPIO_RED);
    yellow_val = gpio_get_value(GPIO_YELLOW);
    green_val = gpio_get_value(GPIO_GREEN);
    
    // build the status string (included pedestrian status)
    status_len = snprintf(status, sizeof(status),
        ...
        yellow_val ? "on" : "off",
        green_val ? "on" : "off",
        pedestrain_call ? "present" : "not present" 
...
static ssize_t device_write(struct file *f, const char __user *buf, size_t len, loff_t *off)
{
    ...
    // Limit how much we copy
    copy_len = len;
    if (copy_len > capacity - *off)
    {
        copy_len = capacity - *off;
    }
    if (copy_len > sizeof(new_cycle) - 1)
    {
        copy_len = sizeof(new_cycle) - 1;
    }

    if (copy_from_user(trafficBuf, buf, copy_len))
    {
        return -EFAULT;
    }

    memcpy(new_cycle, trafficBuf, copy_len);
    new_cycle[copy_len] = '\0';  // Null terminate at actual length

    res = kstrtol(new_cycle, 10, &temp);
    
    // Silently ignore errors - don't return them!
    if (res == -ERANGE || res == -EINVAL)
    {
        return len;  // Success, but ignored
    }

    if (temp < 1 || temp > 9)
    {
        printk(KERN_ALERT "Invalid hz value, must be 1-9\n");
        return len;  // Success, but ignored
    }
    else
    {
    ...
    *off += copy_len;
    trafficBuf_len = *off;
    return len;  // Return original len, not copy_len
}
...
static int __init traffic_init(void)
{
    ...
    ret = request_irq(irq_num_1, btn1_isr, IRQF_TRIGGER_FALLING, "mytraffic_btn1", NULL);
    if (ret) {
...
static void __exit traffic_exit(void)
{
    ...
    kfree(trafficBuf);  // ADD THIS LINE
